...
function getParamsTable(locale: Locale) {
  ...
  return [
    ...
    { name: 'locale', values: 'auto, en, zh', default: 'auto', desc: isZh ? '界面语言；auto 跟随浏览器语言' : 'Interface language; auto follows browser language', required: false },
...
export function EmbedDocsPage() {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.source !== 'aiusage-embed' || data.widget !== selectedWidget || typeof data.height !== 'number') return;
