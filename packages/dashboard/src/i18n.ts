...
export const I18N = {
  en: {
    ...
    claudeCodeDataNotice: 'Claude Code started logging full token telemetry around Dec 2024; earlier activity may be undercounted. Sessions represent API requests (~100–500 per conversation), so Cost / Session reflects per-request cost, not per-conversation cost.',
  ...
  zh: {
    ...
    claudeCodeDataNotice: 'Claude Code 约于 2024 年 12 月起开始记录完整的 Token 遥测数据，更早的活动可能未被完整统计。会话数代表 API 请求数（每次对话约 100–500 次），因此单次费用反映的是单次请求费用，而非单次对话费用。',
