...
export const I18N = {
  en: {
    ...
    claudeCodeDataNotice: 'Claude Code does not retain full historical data — older session logs are periodically cleaned up. Cost and token metrics for earlier periods may be incomplete, which can affect aggregate values like Cost / Session.',
  ...
  zh: {
    ...
    claudeCodeDataNotice: 'Claude Code 不会保留完整的历史数据——旧的会话日志会被定期清理。早期的费用和 Token 数据可能不完整，这会影响单次费用等汇总指标。',
