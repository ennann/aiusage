...
| `force_estimated` | 强制将费用状态标记为 `estimated`；用于本地没有权威账单用量的工具 |
...
- **xAI Grok Build**（`xai/grok-build`）只扫描 `~/.grok/sessions` 中的会话元数据；不读取 `auth.json`，也不会上传聊天内容。本地没有权威的账单用量字段，token 与费用均为估算。

影子价条目使用 `force_estimated: true`，让 calculateCost 始终将 `costStatus` 标为 `estimated`。
